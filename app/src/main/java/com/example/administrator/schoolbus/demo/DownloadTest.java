...
public class DownloadTest {
	...
	private void startDownload(int iUserID)
	{
		...
		String sFileName = "sdcard/pictures";
		//m_lHandle = HCNetSDK.getInstance().NET_DVR_StartDownload(iUserID, dwDownloadType, downloadParam, dwInBufferSize,sFileName.toString());
		m_lHandle = HCNetSDK.getInstance().NET_DVR_StartDownload(iUserID, dwDownloadType, downloadParam,sFileName);
		if (m_lHandle < 0)
		{
	...
	private void StopDownload()
	{
		...
		HCNetSDK.getInstance().NET_DVR_StopDownload((int)m_lHandle);
	...
	private void GetDownloadState()
	{
		...
		HCNetSDK.getInstance().NET_DVR_GetDownloadState((int)m_lHandle,press);
	}
